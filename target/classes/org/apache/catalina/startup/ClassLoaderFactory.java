...
public final class ClassLoaderFactory {
     ...
     * Create and return a new class loader, based on the configuration
     * defaults and the specified directory paths:
     * 创建类加载器，基于repositories参数指定的路径。
     * 也就是说创建的类加载器只负责加载repositories参数路径下的class
     *
     ...
     *                     the class loader. 创建的类加载器负责加载的class的路径，可以是jar文件、目录、URL
     * @param parent Parent class loader for the new class loader, or
     *  <code>null</code> for the system class loader. 新创建的类加载器的父级类加载器，null代表使用system class loader
     * @return the new class loader
     *
     * @exception Exception if an error occurs constructing the class loader
     */
    public static ClassLoader createClassLoader(List<Repository> repositories,
        ...
        // Construct the "class path" for this class loader
        // 为ClassLoader构建classPath
        Set<URL> set = new LinkedHashSet<>();

        if (repositories != null) {
            // 遍历classPath，将路径解析成URL
